...
class MovieRecommender:
    ...
    def compute_cosine_similarity(self, similarity_matrix, liked_movie_ids):
        ...
        try:
            liked_movie_indices = [self.all_movie_ids.index(movie_id) for movie_id in liked_movie_ids if
                                   movie_id in self.all_movie_ids]
            ...
            self.logger.error(f"Error during cosine similarity computation: {str(e)}", exc_info=True)
            return np.array([])
    ...
    def generate_recommendations(self, liked_movie_ids, similarity_matrix):
        ...
        try:
            ...
            self.logger.error(f"Error during recommendation generation: {str(e)}", exc_info=True)
            return []